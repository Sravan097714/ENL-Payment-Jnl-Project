report 50074 "PaymentRemittanceBeforePost"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Report\Layout\VendorPrePaymentJournal.rdl';
    Caption = 'PaymentRemittanceBeforePost';
    ApplicationArea = All;
    ...
    dataset
    {
        dataitem("Gen. Journal Batch"; "Gen. Journal Batch")
        {
            ...
            dataitem("Integer"; "Integer")
            {
                ...
                column(CompanyInformation_Photo; CompanyInformation.Picture)
                {
                ...
                dataitem("Gen. Journal Line"; "Gen. Journal Line")
                {
                    ...
                    column(External_Document_No_GenJnl; "External Document No.")
                    { }
                    ...
                    dataitem("Vendor Ledger Entry"; "Vendor Ledger Entry")
                    {
                        ...
                        column(approvernamevend; approvernamevend) { }
                        column(nam; nam) { }
                        column(sendernam; sendernam) { }
                        column(sendernamevend; sendernamevend) { }
                        column(PurchaseOrderNo; PurchaseOrderNo) { }
                        column(PO_Approved_By; "PO_Approved By") { }
                        column(poapprovedname; poapprovedname) { }
                        column(polastmodified; polastmodified) { }

                        column(approvdate; approvdate) { }

                        trigger OnAfterGetRecord()
                        var
                            postedapprovalentryrec: Record "Posted Approval Entry";
                        ...
                        begin
                            ...
                            postedapprovalentryrec.Reset();
                            postedapprovalentryrec.SetFilter("Document No.", "Document No.");
                            if postedapprovalentryrec.FindFirst then begin
                                approvernamevend := postedapprovalentryrec."Approver ID";
                                approvdate := postedapprovalentryrec."Last Date-Time Modified";
                                sendernamevend := postedapprovalentryrec."Sender ID";
                            end;
                            userrec.Reset();
                            userrec.SetFilter("User Name", approvernamevend);
                            if userrec.FindFirst then
                                nam := userrec."Full Name";

                            userrec.Reset();
                            userrec.SetRange("User Name", sendernamevend);
                            if userrec.FindFirst then
                                sendernam := userrec."Full Name";

                            userrec.Reset();
                            userrec.SetRange("User Name", "PO_Approved By");
                            if userrec.FindFirst then
                                poapprovedname := userrec."Full Name";




                            approvalentryrec.Reset();
                            approvalentryrec.SetFilter("Document No.", PurchaseOrderNo);
                            if approvalentryrec.FindFirst then
                                polastmodified := approvalentryrec."Last Date-Time Modified";

                            /*  userrec.Reset();
                             userrec.SetFilter("User Name", sendernamevend);
                             if userrec.FindFirst then
                                 sendernam := userrec."Full Name"; */
                    ...
                    begin
                        ...
                        clear(username);
                        approvalentriesrec.Reset();
                        approvalentriesrec.SetRange("Document No.", "Document No.");
                        //approvalentriesrec.SetRange(Status, "Approval Status"::Approved); //B2BSRA1.0
                        if approvalentriesrec.FindFirst then begin
                            userrec.Reset();
                            userrec.SetRange("User Name", approvalentriesrec."Sender ID");
                            if userrec.FindFirst then
                        ...
                        approvalentriesrec.Reset();
                        approvalentriesrec.SetRange("Document No.", "Document No.");
                        approvalentriesrec.SetRange("Sequence No.", 1);
                        //approvalentriesrec.SetRange(Status, "Approval Status"::Approved); //B2BSRA1.0
                        if approvalentriesrec.FindFirst then begin
                            approval1id := approvalentriesrec."Approver ID";
                            approval1time := approvalentriesrec."Last Date-Time Modified";
                            userrec.Reset();
                        ...
                        approvalentriesrec.Reset();
                        approvalentriesrec.SetRange("Document No.", "Document No.");
                        approvalentriesrec.SetRange("Sequence No.", 2);
                        //approvalentriesrec.SetRange(Status, "Approval Status"::Approved);
                        if approvalentriesrec.FindFirst then begin
                            sign1id := approvalentriesrec."Approver ID";
                            sign1time := approvalentriesrec."Last Date-Time Modified";
                            userrec.Reset();
                        ...
                        approvalentriesrec.Reset();
                        approvalentriesrec.SetRange("Document No.", "Document No.");
                        approvalentriesrec.SetRange("Sequence No.", 3);
                        //approvalentriesrec.SetRange(Status, "Approval Status"::Approved);
                        if approvalentriesrec.FindFirst then begin
                            sign2id := approvalentriesrec."Approver ID";
                            sign2time := approvalentriesrec."Last Date-Time Modified";
                            userrec.Reset();
