page 50030 "MT101 ENL Payment Approval"
{
    AdditionalSearchTerms = 'print check,payment file export,electronic payment';
    ApplicationArea = Basic, Suite;
    AutoSplitKey = true;
    Caption = 'Signatory Payment Approval';
    DataCaptionExpression = DataCaption;
    DelayedInsert = true;
    PageType = Document;
    PromotedActionCategories = 'New,Process,Report,Bank,Prepare,Approval & Invoices,Page,Post/Print,Line,Account,Check';
    SaveValues = true;
    ...
    layout
    {
        area(content)
        {
            ...
            repeater(Payments)
            {
                ...
                field(vendorbankaccountno; vendorbankaccountno) { Caption = 'Vendor Bank Acc No.'; }
                ...
                field("Last ApproverID"; "Last ApproverID")
                {
                    ApplicationArea = all;
            ...
            part(VendorLedgerEntries; "Vendor Ledger Entries 2")
            {
                ApplicationArea = Basic, Suite;
                Editable = false;
                ...
                //Visible = gboolVisibleVend1;
                Visible = false;

            }
            part(VendorLedgerEntries2; "Vendor Ledger Entries 2")
            {
                ApplicationArea = Basic, Suite;
                Editable = false;
                Enabled = Rec."Document No." <> '';
                SubPageLink = "Applies-to ID" = FIELD("Applies-to ID");
                UpdatePropagation = Both;
                //Visible = gboolVisibleVend2;
                Visible = false;
            }
        ...
        area(factboxes)
        {
            /* part("Attached Documents"; "Document Attachment Factbox")
            {
                ...
                SubPageLink = "Table ID" = CONST(122),
                              "No." = field(DocAttachedID);
                Visible = true;
                //Visible = gboolVisibleVend2;
            }
            part("ApprovedBy"; "Approved By Factbox")
            {
                ApplicationArea = all;
                Caption = 'Approved By';
                SubPageLink = "Document No." = field(DocAttachedID);
            } */
            /* part("Attached Documents "; "Document Attachment Factbox")
            {
                ApplicationArea = All;
...
area(processing)
{
    ...
    begin
        ...
        if venderledgerentryrec.FindFirst() then begin
            /* repeat
                if gtextDocNo <> venderledgerentryrec."Document No." then begin
                    if DocAttachedID = '' then
                        DocAttachedID := venderledgerentryrec."Document No."
                    else
                        DocAttachedID += '|' + venderledgerentryrec."Document No.";

                    gtextDocNo := venderledgerentryrec."Document No.";
                end;
            until venderledgerentryrec.Next = 0 ;
            DocAttachedID := venderledgerentryrec."Document No.";
        end;
    end;
    */
    ...
    */
    ...
    begin
        ...
        if Rec."Applies-to Doc. No." <> '' then begin
            gboolVisibleVend1 := True;
            gboolVisibleVend2 := false;
        end else begin
            gboolVisibleVend2 := True;
            gboolVisibleVend1 := false;
        end;
        venderledgerentryrec.Reset();
        venderledgerentryrec.SetCurrentKey("Entry No.");
        venderledgerentryrec.SetFilter("Applies-to ID", "Document No.");
    ...
    begin
        ...
        vendorbankaccountrec.SetRange("Vendor No.", "Account No.");
        if vendorbankaccountrec.FindSet then begin
            vendorbankaccountno := vendorbankaccountrec."Bank Account No.";
        end;
        vendorrec.Reset();
    ...
    var
        ...
        gboolVisibleVend1: Boolean;
        gboolVisibleVend2: Boolean;
    ...
    begin
        ...
        if PAGE.RunModal(50032, GenJnlBatch) = ACTION::LookupOK then begin
            CurrentJnlBatchName := GenJnlBatch.Name;
            GenJnlMgt.SetName(CurrentJnlBatchName, GenJnlLine);
        end;
    end;
}
