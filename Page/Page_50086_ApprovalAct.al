page 50086 "ApprovalAct Entries"
{
    ApplicationArea = Suite;
    Caption = 'ApprovalAct Entries';
    Editable = false;
    PageType = List;
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field(SenderName; SenderNameGVar)
                {
                    ApplicationArea = all;
                    Editable = false;
                }
                ...
                field(ApproverName; ApproverNameGVar)
                {
                    ApplicationArea = all;
                    Editable = false;
                }
            ...
            part(PurchaseList; "Purchase List For Cue")
            {
                ApplicationArea = All;
                Editable = false;
    ...
    begin
        ...
        if userrec.FindFirst then
            SenderNameGVar := userrec."Full Name"
        else
            Clear(SenderNameGVar);

        userrec.Reset();
        userrec.SetRange("User Name", "Approver ID");
        if userrec.FindFirst then
            ApproverNameGVar := userrec."Full Name"
        else
            Clear(ApproverNameGVar);
